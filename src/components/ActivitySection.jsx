...
export default function ActivitySection() {
  ...
  const card1CustomNotes = {
    ...
    'c1-a-delete-vehicle': ['#Can do over email', '#Make sure all drivers assigned to vehicle', '#Can only backdate if', 'Total loss ( effective 1 day after total loss, or 1 day after rental was returned)' , 'Sold', 'Got insurance elsewhere (needs proof with pol # & effective date)', 'Call claims department if backdating due to total loss & confirm date of loss & if a rental taken or when rental was returned', 'If backdate > 30 days need to email it to UW'],
    ...
    'c1-snb-new-business-auto': ['DASH', 'Request MVR via portal', 'Signed App', 'Signed TPL form if TPL only', 'Signed client consent', 'Signed MAC if monthly payments', 'VOID cheque', 'Any signed forms for declinded coverages if needed', 'VIR if > 12yrs old for coll / comp', '#Set abeyance to SNB - 3 Support', '#If IPFS collect downpayment & update payment sheet', '#Get credit consent over phone before letting client go', '#On pink slip use quote # as binder #', '#Only Wawanesa if property is with them', '#SGI TPL only requires TL approval', '#If Grid rated, advise it may take 2-3 weeks'],
    'c1-snb-new-business-property': ['Signed App', 'Signed client consent', 'Any signed forms for declinded coverages if needed', 'Signed MAC if monthly payments',  'VOID cheque', 'iClarify for Homeowners & bareland condo', '#Set abeyance to SNB - 3 Support', '#Get credit consent over phone before letting client go'],
  ...
  const card3CustomSummaries = {
    ...
    'c3-a-delete-vehicle': ['#Can do over email', '#Make sure all drivers assigned to vehicle', '#Can only backdate if', 'Total loss ( effective 1 day after total loss, or 1 day after rental was returned)' , 'Sold', 'Got insurance elsewhere (needs proof with pol # & effective date)', 'Call claims department if backdating due to total loss & confirm date of loss & if a rental taken or when rental was returned', 'If backdate > 30 days need to email it to UW'],
    ...
    'c3-snb-new-business-property': [
                                     'Signed App', 'Signed client consent', 'Any signed forms for declinded coverages if needed', 'Signed MAC if monthly payments',  'VOID cheque', 'iClarify for Homeowners & bareland condo',
                                     '#Set abeyance to SNB - 3 Support', '#Get credit consent over phone before letting client go'
  ...
  if (lower.includes('change payment date')) {
    ...
    if (lower.includes('a - cancellation')) {
      return [
        ...
        { name: 'Xln Fees', label: 'Advised Xln Fees', type: 'select' , options: ['Yes', 'No', 'No fees'] },
    ...
    if (lower.includes('payment over phone')) {
      return [
        ...
        { name: 'amount', label: 'Amount', type: 'text' , placeholder: '$333' },
        { name: 'confirmed by', label: 'Confirmed By', type: 'text' , placeholder: 'Travis Mounsteven' },
        { name: 'notes', label: 'Notes', type: 'text', placeholder: 'None' },
      ]
    }
  ...
  // SNB New Business Property: selected type per-key (Tenant | Home | Condo)
  const [snbTypeByKey, setSnbTypeByKey] = useState({ 'c1-snb-new-business-property': 'Tenant' })
  const handleSnbTypeChange = (key, val) => setSnbTypeByKey((p) => ({ ...p, [key]: val }))

  const snbTypeNotes = {
    Tenant: ['Verify lease/tenancy', 'Request proof of tenancy (lease agreement)', 'Confirm mailing address'],
    Home: ['Request property details', 'iClarify for Homeowners if required', 'Obtain dwelling details and utilities'],
    Condo: ['Request condo docs', 'iClarify for Condo if required', 'Get condo corp contact and fees info'],
  }
    ...
    try {
      ...
      if (schema && schema.length) {
        schema.forEach((f) => {
          ...
          if (raw && (/^(effectiveDate|effDate|effevticeDate|date)$/i.test(f.name) || (/^date$/i.test(String(f.name)) && /date of call/i.test(String(f.label || ''))))) {
            try {
              const months = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']
              const s = String(raw)
          ...
          <div className="note-card">
            <div className="card-header">Notes</div>
              <div className="card-body">
                    ...
                    {activeCard1?.key === 'c1-snb-new-business-property' && (
                      <div className="snb-type-control" style={{ marginTop: 10 }}>
                        <div style={{ display: 'block', marginBottom: 6 }}>
                          <label style={{ marginRight: 8 }}></label>
                          <CustomSelect
                            options={[{ key: 'Tenant', label: 'Tenant' }, { key: 'Home', label: 'Home' }, { key: 'Condo', label: 'Condo' }]}
                            value={snbTypeByKey[activeCard1.key] || 'Tenant'}
                            onChange={(k) => handleSnbTypeChange(activeCard1.key, k)}
                          />
                        </div>
                        <ul className="snb-custom-notes">
                          {(snbTypeNotes[snbTypeByKey[activeCard1.key] || 'Tenant'] || []).map((n, i) => (
                            <li key={i}>{n}</li>
                          ))}
                        </ul>
                      </div>
